    ...
    >
      <CarouselContent className="w-full h-[190px] max-h-full ">
        {ALTER8_FEATURES.map((feature, index) => (
          <CarouselItem
            key={index}
            className="w-full h-full max-h-full basis-2/5 "
          >
...
function InvestmentCard(props: IInvestmentCard) {
  return (
    <div className="h-[200px] w-[290px]">
      <Card
        className={`w-full h-full max-h-full border-4 border-[#F5F5F5] ${
      ...
      >
        <CardContent className="w-full h-full flex items-center justify-center p-5">
          <div className="flex justify-between flex-col gap-3  w-[70%] h-full ">
            ...
            <p className="text-lg leading-[10px]">
              {props.investmentRateDescription}
            </p>
    ...
    <div className=" w-full h-full px-10 py-5">
      ...
      <div className="p-1 w-3/4 h-2/5 flex items-center gap-10 ">
        ...
        <div className="flex h-[50%] items-end">
          <Button className="w-[290px] h-[60px] rounded-full">
            Show Me Returns
          </Button>
        </div>
      </div>
...
export const WhySection = () => {
    ...
    const [showLearnMore, setShowLearnMore] = useState(true); // New state to control "Learn more" visibility
  
    // Function to handle the "Learn more" click
    const handleLearnMoreClick = (e: { preventDefault: () => void; }) => {
      e.preventDefault();  // Prevents the page refresh or navigation
      setShowTestimonials(true);
      setShowLearnMore(false);  // Hide "Learn more" after it's clicked
    };
  
    return (
      <div className="bg-[#F7F7FF] w-full px-10 py-10">
        <div className="flex mt-5 flex-col gap-3">
          <h1 className="text-31xl font-semibold leading-tight mt-0 mb-2">
            Why Altern8
          </h1>
          <p>
            At Altern8 Club, our experienced team empowers you to invest in real
            estate through innovative fractional ownership and high-yield
            fixed-income products tailored to your financial goals. Ensuring you
            can confidently grow your wealth.
          </p>
          ...
          <div className="mt-10 flex flex-col gap-1">
            <h2 className="text-21xl font-semibold">
              Real Estate Developer Due Diligence stack with 70+ micro risk
              factor analysis
            </h2>
            ...
            <div className="p-2 mt-7 flex gap-10 items-center">
              {ALTERN8_SERVICES.map((feature, index) => (
                <div
                  className="flex justify-between flex-col gap-2 flex-grow"
                  key={index}
                >
            ...
            {showLearnMore && (
              <Link href="#" className="hover:underline" prefetch={false}>
                <div
                  className="mt-5 flex gap-3 items-center cursor-pointer"
                  onClick={handleLearnMoreClick} // Set the state to true when clicked
                >
